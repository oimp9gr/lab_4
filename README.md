...
возвращает его представление в виде строки. При этом вам запрещено в явном виде использовать символы и строки,
...
Кроме того вам нужно написать функцию <code>PrintDivisionResult</code>, которая будет принимать два 
...
string PrintDivisionResult(Number & lhs, Number & rhs);
...
int main() {
    ...
    Number n3 = n1.Add(n1);
    cout << n3.GetData(); // 8
    cout << PrintDivisionResult(n1, n2) // divide by zero;
    return 0;
}
```
...
Использовать std::list запрещено. Поэтому 
